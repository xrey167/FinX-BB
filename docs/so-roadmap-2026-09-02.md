...
|---|---|---|---|
| Selectivity | The targeted cell's answer disappears for direct, paraphrased, multi-hop and reverse access; controls unchanged | REVOKE: routing removal by mask (**by construction**, F1). SHRED: payload stays routable, the *learned* marker gate refuses it (**learned**, F3; F4 with the verified gate of E-000010) — **and E-000029 measures what that gate certifies**: its operational radius is 0.90 on all eleven checkpoints against the store's declared 0.35, the annulus the store calls deleted is accepted at 1.0000 over 2.2M markers, and the published 8.49e-04 is the rate on a distribution that excludes that annulus. A learned gate certifies the margin between the classes it was shown, not the predicate. **E-000030** then asks the question without an attack at all: sweeping every value the payload could hold, REVOKE is certified at the outputs and not at the encoding, SHRED at neither, and only DELETE — which takes the row out of the bank — is independent by construction | E4 synthetic |
...
| Reconstruction resistance | With the unsupervised gate, SHRED leaves a residual that a probe and forced choice exploit (E-000004); with the class-balanced verified gate (E-000010) probe, forced choice and logit rank are at chance and the value contribution is zero while the payload is still present and routed to; dependency closure is required for derivable facts. **E-000028 narrows all of this to the VALUE channel**: the routing keys are computed before the gate and k_rev is a function of the object, so a candidate sweep through reverse queries recovers the shredded object at 1.0000 over 500 targets, with numbers identical to the un-shredded cell's. REVOKE and DELETE, which take the row out of routing, are at chance (0.0040, mean rank 128.0). | learned (SHRED column, value channel only); after REVOKE the activation rows are the mask | E4, F4 within the synthetic system **on the value channel** (E-000010) — **E-000019 confirms this out of sample and at chance**: on three seeds that took no part in choosing the configuration, forced choice is exactly 375 of 750 pooled trials and the probe 4 of 750 against a chance of 1 in 256, with every exact interval containing its chance level |
| Shared knowledge objects | **E-000025 corrects the frozen-GPT-2 half of this row**: E-000020's recorded reading (direct 0.5667, alias 0.5067) is template 0 only, and re-scoring the same checkpoints at all twelve phrasings gives direct 0.9989 and alias 0.9250 on the held-out template 10, with sharing costing 0.0954 and link training 0.0688 on the worst seed. E-000015: link cells give several access keys ONE object. One UPDATE or one SHRED reaches every path (100% / 100% unknown) against 0% / copies fully readable in the duplication arm; after that one SHRED the object is unrecoverable through every alias (probe 0.7%, forced choice 0.503) and fully recoverable through the copies (probe 87.3%, forced choice 1.000); provenance names alias and target | synthetic system only; shredding the alias itself reaches 93% on the worst seed; chains resolve once chains are in training (E-000016: two slots 100%, one slot refuses 100%) | E4 / **F3** |
...
4. Prior conflict: real entities with counterfactual objects (Berlin → Switzerland). Measure override during ACTIVE and fallback to the pretrained distribution after REVOKE (KL to the base model below 0.05 nats). State explicitly that the pretrained fact is not deleted. **Recorded (E-000013):** on the trained template the override is complete (100%) against a prior that names the true capital 96% of the time, and after REVOKE or SHRED the distribution returns to the pretrained one at 0.0004 nats with the paired excess over the base model at 0.0000 for probe, forced choice and counterfactual top-1. The pretrained fact is explicitly not deleted. Item 5 of this stage is untouched, and the same held-out weakness appears here as injection into generic text (2.27 nats against a bar of 0.05).
...
5. Held-out-template deletion below 95% with train-template deletion ≈ 100% (Stage 2): surface overfitting. **This criterion has FIRED.** E-000012 answers ' unknown' after REVOKE at 98% on the trained template and 52% on the weakest held-out one (E-000011: 70% / 16%). Two qualifications belong with it, and neither rescues it: the budget was 2 trained templates against the 8 this stage prescribes, and the failure is one of REFUSAL, not of recovery — after REVOKE the deleted object is never the top-1 answer on any held-out template (0.0 in all three seeds); the model names some other entity. Reading on those same templates is already unreliable while the cell is ACTIVE (56% correct, 7% unknown on template 4), so reading and refusal have to be separated before the criterion can be adjudicated. **Both follow-ups are now recorded.** E-000017-A separated reading from refusal: conditioned on the model having read the fact at all, refusal on held-out phrasings is 96.1% and the deleted object returns in 0.15% of those cases, so the defect sits in the query and routing path. E-000017-B then gave the stage its prescribed budget of eight trained templates: unconditional refusal rose to 89.8% (worst seed 86.5%) from about 52%, the conditional figure to 99.3%, and the deleted object returned in exactly 0.0000 of cases in every seed and on every held-out template. The criterion is still NOT cleared at its own 95% bar, but it is no longer evidence against the deletion mechanism. Two problems take its place: reading on held-out phrasings (74.0% against a 90% bar) and, more seriously, injection where there is no key — generic sentences move by 3.27 nats against a 0.05 bar and the figure got WORSE with more templates, not better, so the layer perturbs the frozen model on text it has no business touching.
