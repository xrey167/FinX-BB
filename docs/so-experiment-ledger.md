...
| E-000018 | Can the adapter be stopped from injecting into text it has no key for? | Not in this design, and the three-arm ablation says why. Adding the CAPACITY to inject nothing (an absolute match score against the best real cell key) changes nothing at all: 3.2681 nats against a baseline of 3.2741. Training the BEHAVIOUR on generic sentences brings it to 0.6035, and both together to 0.6736, so all of the improvement is the training and none is the capacity. Every arm stays a factor of twelve above the 0.05 bar, and the two that improve it pay with reading on held-out phrasings (68.9% against 74.0%) and refusal (88.0% against 89.8%). The reason is structural: answering ' unknown' when a cell is gone needs an INJECTION, changing nothing on text with no key needs NONE, and both are routed through the same null column, so a gate that closes on prose also closes on a broken question and takes the refusal with it. The separation of the two channels is E-000022 | E5 (substrate) / F1, negative with a mechanism |
...
### 31.6 Symlink: which half of the concept is implemented
