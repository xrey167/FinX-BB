...
class KnowledgeAdapterLM(nn.Module):
    ...
    @property
    def last_injected(self):
        return getattr(self._request_state, "last_injected", None)

    @last_injected.setter
    def last_injected(self, value):
        self._request_state.last_injected = value
    ...
    def forward(self, bank: Optional[Dict[str, torch.Tensor]], input_ids: torch.Tensor, attention_mask: torch.Tensor,
        ...
        with self._memory_request(bank, last_idx, cell_mask) as ctx:
            ...
            if ctx is not None and ctx.get("deferred"):
            ...
            # Kept as per-request attributes so existing four-tuple call sites still work.
            self.last_query = torch.stack(ctx["query"], dim=1) if ctx is not None and ctx.get("query") else None
            self.last_injected = (torch.stack(ctx["injected"], dim=1)
                                  if ctx is not None and ctx.get("injected") else None)
