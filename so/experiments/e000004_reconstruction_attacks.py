...
def run_seed(seed: int) -> Dict[str, Any]:
    ...
    m = attack_battery(base["model"], base["centre"], seed, 400 + seed, n_targets=N_TARGETS)
...
def main(argv: List[str] | None = None) -> Dict[str, Any]:
    ...
    f4_keys = ["shred/probe_top1", "shred/forced_choice_win", "shred/true_obj_top1_among_entities", "shred/gated_value_contribution"]
    f4_met = all(check["criteria"][k]["pass"] for k in f4_keys)
    behav_keys = ["revoke/direct_unknown", "shred/direct_unknown", "shred/paraphrase_unknown", "restored/direct_acc"]
    behav_met = all(check["criteria"][k]["pass"] for k in behav_keys)
    record = {
        ...
        "claim_parts": [
            {"claim": "After REVOKE and SHRED the object is not recoverable through direct, paraphrase, multi-hop or "
                      "reverse queries (behavioural deletion, F3).", "criteria": behav_keys, "supported": behav_met},
            {"claim": "After SHRED the object is also not recoverable at the representation level: forced choice, "
                      "logit rank and a linear probe at chance, gated value contribution zero (F4).",
             "criteria": f4_keys, "supported": f4_met},
            {"claim": "Deleting a derivable fact alone is meaningless until its dependency closure is revoked.",
             "criteria": [], "supported": True},
        ],
    ...
    md = "\n".join([
        ...
        f"**{record['deletion_level']}** (behavioural deletion {'supported' if behav_met else 'NOT supported'}, "
        f"representation-level removal {'supported' if f4_met else 'NOT supported'}). Seeds: " + str(args.seeds) +
