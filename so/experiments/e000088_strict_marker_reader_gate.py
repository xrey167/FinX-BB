...
No CAVI/novelty interpretation is permitted unless all three requested training seeds eventually
reach fresh real-symlink correctness >=0.95. Historical E77 remains 0.99/0.95/0.94; this run does
not rewrite it. The only setup correction is that samples labelled mechanically valid must satisfy
...
def main():
    ...
    requested_gate=all(r['qualified_for_attack_interpretation'] for r in rows)
    rec={'experiment':'E-000088','title':'strict-marker E77 reproduction','rows':rows,
         'requested_seed_gate':requested_gate,
         'all_three_seed_gate':len(rows)>=3 and requested_gate,
         'breakthrough':False,'novelty_claim':False,
         'historical_e77_preserved':'0.99/0.95/0.94 remains failed evidence',
         'boundary':'Per-seed CI success is not three-seed promotion. Combine seeds 0,1,2 before interpreting CAVI. Full-vocabulary/heldout/lifecycle/locality/J-lens gates remain separate.'}
    ...
    if not requested_gate: raise SystemExit(3)
